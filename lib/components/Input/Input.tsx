...
export const Input = forwardRef<HTMLInputElement, InputProps>(({
    ...
    const onClickClearButton = () => {
        const inputElement = (ref && 'current' in ref ? ref.current : inputRef.current) as HTMLInputElement | null;
    ...
    return (
        ...
        >
            {({ disabled, hover, autofocus }) => (
                <div
                    className={clsx(bemClass(), bemClass('size', size), { [bemClass('', 'error')]: error })}
                    autoFocus={autofocus}
                    {...(isInputFocused && { 'data-focus': isInputFocused })}
                    {...(hover && { 'data-hover': hover })}
                    {...(disabled && { 'data-disabled': disabled })}
                >
                    <div className={clsx(bemClass('action-box'))} data-active={String(!!prependIcon)}>
                        {prependIcon}
                    </div>
                    <input
                        ...
                        onBlur={onBlur}
                    />
                    <div className={clsx(bemClass('action-box'))} data-active={String(clearButton || loading)}>
                        {clearButton &&
